...
- Run `cx --relations` or inspect `~/.cx/relations` to see which symbol pairs and triads co-occur most and consider minting compound tags.
...
- The wrapper now mines queued prompts under `~/.cx/offline` and per-symbol context logs for repeated phrases, but keep reviewing these files to grow and refine the dictionary over time.
