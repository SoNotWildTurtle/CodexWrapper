...
- Add `cx --recommendations` to my routine so I see the backlog summary and raw entries in `recommendations.md` before each sweep; the count of planned items should stay comfortably ahead of completed work. Use `--recommendations-filter`/`--recommendations-search` to zoom in on the next area of focus without losing the surrounding context.
