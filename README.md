...
- `--recommendations` parse `recommendations.md`, print a status summary (counts of Planned vs Completed), and then emit the full backlog so local or remote agents can triage without opening the repository manually. Combine `--recommendations-filter=<status>` (for example `Planned`, `In Progress`, `Completed`) and `--recommendations-search=<term>` flags to focus on a subset of entries; multiple filters and searches can be supplied, and filtered output still preserves the backlog header and auto-generated summaries for context.
