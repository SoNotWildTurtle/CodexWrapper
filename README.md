...
- Add `--config-scope=user` (PowerShell: `-ConfigScope user`) to target `${CX_HOME:-~/.cx}/config` instead of the project file.
  ...
  - `--select-root` opens a cross-platform folder picker (macOS `osascript`, Zenity/KDialog/Yad on Linux when available, WSL/
    Windows shells via `powershell.exe`, and otherwise Tkinter or a manual prompt) so you can choose the project interactively;
    PowerShell mirrors this via `Invoke-Codex -SelectProjectRoot`.
